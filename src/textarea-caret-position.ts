...
export function getCaretCoordinates(element: HTMLTextAreaElement | HTMLInputElement, position: number, options?: CaretOptions): CaretCoordinates {
	...
	let clampedPosition = Math.max(0, Math.min(position, element.value.length));
	...
	COPIED_PROPERTIES.forEach((prop) => {
		if (isInput && prop === "lineHeight") {
			...
			const value =
				computedStyle.getPropertyValue(prop) || computedStyle.getPropertyValue(`-webkit-${prop}`) || computedStyle.getPropertyValue(`-moz-${prop}`);
			if (value) {
